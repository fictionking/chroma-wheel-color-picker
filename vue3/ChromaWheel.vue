<template>
  ...
  <div class="color-picker-container" ref="colorPickerContainer" :style="{ backgroundColor: backgroundColor }">
    ...
    <svg style="z-index: 1;position:absolute;left: 2px;top: 2px;" width="76" height="116"
      xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve"
      overflow="hidden">
      <defs>
        <clipPath id="clip0">
          <path
            d="M515.84 296.827C508.136 296.827 501.891 303.057 501.891 310.743 501.891 318.429 508.136 324.66 515.84 324.66 523.544 324.66 529.789 318.429 529.789 310.743 529.789 303.057 523.544 296.827 515.84 296.827ZM532.288 224.029C515.877 224.029 502.574 237.301 502.574 253.673 502.574 270.045 515.877 283.317 532.288 283.317 548.698 283.317 562.001 270.045 562.001 253.673 562.001 237.301 548.698 224.029 532.288 224.029ZM494 216 570 216 570 332 494 332Z"
      ...
      <g transform="translate(-494 -216)">
        <g clip-path="url(#clip0)">
          <!-- 金属基底 -->
          <path
            d="M551.529 227.764C571.82 227.764 588.268 244.212 588.268 264.501 588.268 277.182 581.843 288.362 572.07 294.964L568.627 296.834 563.718 298.844C560.513 300.874 557.607 304.319 555.755 308.702 553.903 313.082 553.462 317.571 554.241 321.284L554.32 322.906 554.253 325.767C552.441 334.624 544.602 341.291 535.207 341.291 524.468 341.291 515.765 332.585 515.765 321.849 515.765 319.163 516.309 316.604 517.294 314.278L518.197 312.94 519.333 311.581C521.973 308.856 523.991 304.826 524.764 300.133 525.924 293.094 523.746 288.052 520.142 283.288L517.68 278.801C515.819 274.406 514.792 269.573 514.792 264.501 514.792 244.212 531.241 227.764 551.529 227.764Z"
            fill="url(#metalGradient)" stroke="rgba(0,0,0, 0.5)" stroke-width="2.07682" stroke-miterlimit="8"
            fill-rule="evenodd" transform="matrix(1.00234 0 0 1 -20.741 -10.5625)" />
          <!-- 金属光泽 -->
          <path
            d="M551.529 227.764C571.82 227.764 588.268 244.212 588.268 264.501 588.268 277.182 581.843 288.362 572.07 294.964L568.627 296.834 563.718 298.844C560.513 300.874 557.607 304.319 555.755 308.702 553.903 313.082 553.462 317.571 554.241 321.284L554.32 322.906 554.253 325.767C552.441 334.624 544.602 341.291 535.207 341.291 524.468 341.291 515.765 332.585 515.765 321.849 515.765 319.163 516.309 316.604 517.294 314.278L518.197 312.94 519.333 311.581C521.973 308.856 523.991 304.826 524.764 300.133 525.924 293.094 523.746 288.052 520.142 283.288L517.68 278.801C515.819 274.406 514.792 269.573 514.792 264.501 514.792 244.212 531.241 227.764 551.529 227.764Z"
            fill="url(#metalHighlight)" fill-rule="evenodd" transform="matrix(1.00234 0 0 1 -20.741 -10.5625)" />
        </g>
      </g>
    </svg>
    ...
    <div class="favorite-colors-bottom">
      <div v-for="(color, index) in favoriteColors.slice(0, 5)" :key="'fav-bottom-' + index" class="fav-color"
        ...
        @click="handleFavoriteClick(index)" @contextmenu.prevent="removeFavorite(index)" :draggable="color !== null"
        ...
        <i v-if="!color" class="fa fa-plus"></i>
      </div>
    </div>
    <!-- 收藏的颜色-右侧 -->
    <div class="favorite-colors-right">
      <div v-for="(color, index) in favoriteColors.slice(5, 10)" :key="'fav-right-' + index" class="fav-color"
        ...
        @touchstart="handleTouchStart($event, index + 5)" @touchmove="handleTouchMove($event)"
        @touchend="handleTouchEnd($event, index + 5)">
        <i v-if="!color" class="fa fa-plus"></i>
      </div>
    </div>

    <div id="lockSaturation" title="锁定饱和度" class="lock-saturation" @click="toggleSaturationLock">
      <i :class="isSaturationLocked ? 'fa fa-lock' : 'fa fa-unlock'" style="font-size:8px;"></i>
    </div>
    <!-- 背景色切换区域 -->
    ...
    <div class="color-input-section">
      <div class="input-container">
        <select id="color-format-select" class="color-format-select" v-model="colorFormat" @change="handleColorFormatChange">
        ...
        <input type="text" id="color-value-input" v-model="colorValue" class="color-value-input" @input="handleColorValueInput" />
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const updateMarkerPositions = () => {
      ...
      const wheelMarkerEl = document.getElementById('wheel-marker');
      if (wheelMarkerEl) {
        wheelMarkerEl.style.left = `${pos.x}px`;
        wheelMarkerEl.style.top = `${pos.y}px`;
        wheelMarkerEl.style.display = 'block';
        const wheelRgb = ColorUtils.hslToRgb(h, s, 50); // 使用固定亮度值50%，与色盘一致
        wheelMarkerEl.style.backgroundColor = `rgb(${wheelRgb.r}, ${wheelRgb.g}, ${wheelRgb.b})`;
      }

      // 饱和度环弧标记 - 根据当前饱和度值计算角度位置
      const saturationMarkerEl = document.getElementById('saturation-marker');
      if (saturationMarkerEl) {
        ...
        saturationMarkerEl.style.left = `${saturationPosX}px`;
        saturationMarkerEl.style.top = `${saturationPosY}px`;
        saturationMarkerEl.style.display = 'block';
      }

      // 亮度环弧标记 - 根据当前亮度值计算角度位置
      const lightnessMarkerEl = document.getElementById('lightness-marker');
      if (lightnessMarkerEl) {
        ...
        lightnessMarkerEl.style.left = `${lightnessPosX}px`;
        lightnessMarkerEl.style.top = `${lightnessPosY}px`;
        lightnessMarkerEl.style.display = 'block';
      }

      // 透明度环弧标记 - 根据当前透明度值计算角度位置
      const alphaMarkerEl = document.getElementById('alpha-marker');
      if (alphaMarkerEl) {
        ...
        alphaMarkerEl.style.left = `${alphaPosX}px`;
        alphaMarkerEl.style.top = `${alphaPosY}px`;
        alphaMarkerEl.style.display = 'block';
      }
      ...
      // 复用已声明的变量，不再重复声明
      if (saturationMarkerEl) {
        saturationMarkerEl.style.backgroundColor = `rgb(${rgb.r}, ${rgb.g}, ${rgb.b})`;
      }
      if (lightnessMarkerEl) {
        lightnessMarkerEl.style.backgroundColor = `rgb(${rgb.r}, ${rgb.g}, ${rgb.b})`;
      }
      if (alphaMarkerEl) {
        alphaMarkerEl.style.backgroundColor = `rgb(${rgb.r}, ${rgb.g}, ${rgb.b})`;
      }
    };
    ...
    const handleFavoriteClick = (index) => {
      if (favoriteColors.value[index]) {
        ...
        favoriteColors.value[index] = currentRgba;
    ...
    const initEventListeners = () => {
      ...
      document.addEventListener('mousedown', (e) => {
        if (e.target === colorSelector.value || colorSelector.value.contains(e.target)) {
          handleColorSelectorInteraction(e, false);
        }
      });

      document.addEventListener('mousemove', (e) => {
        if (isDraggingWheel.value || isDraggingRing.value) {
          handleColorSelectorMove(e, false);
        }
      });
      ...
      document.addEventListener('touchstart', (e) => {
        if (e.target === colorSelector.value || colorSelector.value.contains(e.target)) {
          handleColorSelectorInteraction(e, true);
        }
      }, { passive: false });

      document.addEventListener('touchmove', (e) => {
        if (isDraggingWheel.value || isDraggingRing.value) {
          handleColorSelectorMove(e, true);
        }
      }, { passive: false });
      ...
      window.addEventListener('resize', () => {
        nextTick(() => {
          if (colorSelector.value && colorWheel.value) {
    ...
    return {
      ...
      selectPresetColor,
      resetToInitialColor,
      toggleSaturationLock,
      handleBgColorClick,
      handleBgColorTouchStart,
...
.bg-color-slider {
  ...
  top: 140px;
  left: 10px;
  background: linear-gradient(to bottom, #9797977f 0%, #9797977f 25%, #FFFFFF 25%, #FFFFFF 50%, #808080 50%, #808080 75%, #000000 75%, #000000 100%);
  box-shadow: 0 0 2px 2px rgba(0, 0, 0, 0.2);
  z-index: 10;
